...
/* Массив int для хранения результатов опроса датчиков для последующей передачи в Master-контроллер через i2c
  ...
  3 - температура термопары
*/
uint8_t SlaveResult[4] = {0, 0, 0, 0};
...
void requestEvent()
{
  Wire.write(SlaveResult, 4);         // ответ на запрос от Master-контроллера
}
...
{
  HotWaterTankLevel = 1;  // пока нет готового датчика временно передаем значение 1
...
{
  SlaveResult[3] = thermocouple.readCelsius();
}
